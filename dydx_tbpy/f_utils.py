...
def get_ISO_times(entry_time, resolution):
    ...
    Multiplier, Divider, TimeFreq = get_time_frequency(resolution)

    # Calculate candles needed
    now = convert_time(datetime.now() - timedelta(seconds=28800))
    time_diff = now - entry_time
    diff = math.ceil(time_diff.total_seconds() / Divider) * Multiplier
    ...
    if TimeFreq == "h":
        for i in range(0, int(total_call)):
            date_start.append(date_start[-1] - timedelta(hours=100))
...
def get_time_frequency(resolution):
    ...
    return Multiplier, Divider, TimeFreq
